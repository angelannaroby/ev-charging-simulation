...
export function runSimulation(inputs: SimulationInputs): SimulationResult {
  ...
  const random = inputs.useSeededRandomness ? seedrandom(DETERMINISTIC_SEED) : Math.random
  ...
  for (let tick = 0; tick < totalTicks; tick += 1) {
    ...
    const baseHourlyProbability =
      ARRIVAL_PROBABILITIES_BY_HOUR[hourOfDay] ?? ARRIVAL_PROBABILITIES_BY_HOUR[0]
    ...
    for (const charger of chargers) {
      ...
      charger.remainingEnergyKwh = (distanceKm / 100) * inputs.carConsumptionKwhPer100Km
    ...
    for (const charger of chargers) {
      ...
      const deliveredEnergyKwh = Math.min(charger.remainingEnergyKwh, energyPerTickKwh)
  ...
  const concurrencyFactor = theoreticalPeakKw > 0 ? actualPeakKw / theoreticalPeakKw : 0
